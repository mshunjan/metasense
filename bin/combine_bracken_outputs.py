...
def main():
    ...
    parser.add_argument(
        "--files", dest="files", nargs="+", type=str, required=True, help="Bracken output files to combine."
    )
    parser.add_argument(
        "--names",
        dest="names",
    ...
    parser.add_argument(
        "-o", "--output", dest="output", required=True, help="Name of output file with combined Bracken results."
    )
    ...
    if len(args.names) == 0:
        for f in args.files:
            curr_sample = os.path.basename(f).split(".")[0]
    ...
    for f in args.files:
        ...
        for line in i_file:
            ...
            if name not in sample_counts:
                ...
                sys.exit(
                    "Taxonomy IDs not matching for species %s: (%s\t%s)" % (name, taxid, sample_counts[name].keys()[0])
                )
            if len(level) == 0:
                level = taxlvl
            elif level != taxlvl:
